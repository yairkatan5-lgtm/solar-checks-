...
export default function DashboardTabs({ activeTab, onTabChange, hasPeriodView, hasSolar, isSynthesized }) {
  ...
  return (
    <div className="sticky top-16 z-20 bg-white/95 backdrop-blur border-b border-brand-ink-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-2.5">
        <div className="flex flex-wrap gap-2 justify-start" role="tablist" aria-label="אזורי דשבורד">
          {items.map((t) => (
            <button
              key={t.id}
              type="button"
              ...
              className={`text-sm font-extrabold rounded-full px-4 py-2 transition border ${
                activeTab === t.id
